...
let elcolors = tptcolors.map((val) => {
	return getColor(val);
});
		...
		for (y in colors) {
			for (x in colors[y]) {
				...
				if (elems) {
					...
					c = matches[j]
					if (preview) {
						ctx.fillStyle = 'rgb('+elcolors[c].join(',')+')'
						ctx.fillRect(+x+xf, canvas.height-(+y+yf), 1, 1)
					}
				}
				else {
					c = defaultElem
				...
				//Set GPMP life to 0
				else if (c == 154) script.push(`a = sim.partCreate(-3, ${x}, ${offsettedY}, ${c})\nsim.partProperty(a, sim.FIELD_LIFE, 0)`)
				//Set CLST tmp to 5
